...
/*
this function will be implemented by each AI script
It allows for protection of the Robot class as well as giving the AI all the relevant information.
 */
...
/**
 * remove the DOM elements related to this level, clear timeouts and kill itself.
...
function run(level) {
	...
	setTimeout(function() {
		level.map.update();
		level.testVictory();
		if (level.map.endGame) {
			level.render();
			alert(level.map.endMessage);
		} else {
			level.render();
			run(level);
		}
